...
contract Farming is IFarming, ReentrancyGuardUpgradeable, OwnableUpgradeable {
  /// @notice Libraries
  ...
  /// @dev Get token from mToken
  modifier borrowFromMToken(address mToken, uint256 value) {
    IMToken(mToken).transferUnderlyingTo(address(this), value);
    _;
  }
  ...
  /// @param mToken The address of the mToken to borrow.
  ...
  function work(
    ...
    borrowFromMToken(mToken, borrowAmount) 
    accrue(principalAmount) 
    nonReentrant 
  {
    _work(id, mToken, worker, principalAmount, borrowAmount, maxReturn, data);
  }
