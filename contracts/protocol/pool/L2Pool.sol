...
import {CalldataLogic} from '../libs/logic/CalldataLogic.sol';
...
contract L2Pool is Pool, IL2Pool {
  ...
  /// @inheritdoc IL2Pool
  function repayWithMTokens(bytes32 args) external override returns (uint256) {
    (address asset, uint256 amount, uint256 interestRateMode) = CalldataLogic.decodeRepayParams(
      _reservesList,
      args
    );

    return repayWithMTokens(asset, amount, interestRateMode);
  }
  ...
  function liquidationCall(bytes32 args1, bytes32 args2) external override {
    ...
    liquidationCall(collateralAsset, debtAsset, user, debtToCover, receiveMToken);
  }
}
