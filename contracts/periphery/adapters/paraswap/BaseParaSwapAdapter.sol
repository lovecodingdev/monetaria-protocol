...
abstract contract BaseParaSwapAdapter is FlashLoanSimpleReceiverBase, Ownable {
  ...
  /**
   * @dev Get the mToken associated to the asset
   * @return address of the mToken
   */
  function _getReserveData(address asset) internal view returns (DataTypes.ReserveData memory) {
    return POOL.getReserveData(asset);
  }

  function _pullMTokenAndWithdraw(
    address reserve,
    address user,
    uint256 amount,
    PermitSignature memory permitSignature
  ) internal {
    IERC20WithPermit reserveMToken = IERC20WithPermit(
      _getReserveData(address(reserve)).mTokenAddress
    );
    _pullMTokenAndWithdraw(reserve, reserveMToken, user, amount, permitSignature);
  }

  /**
   * @dev Pull the MTokens from the user
   * @param reserve address of the asset
   * @param reserveMToken address of the mToken of the reserve
   * @param user address
  ...
  function _pullMTokenAndWithdraw(
    address reserve,
    IERC20WithPermit reserveMToken,
    address user,
    uint256 amount,
    PermitSignature memory permitSignature
  ) internal {
    ...
    reserveMToken.safeTransferFrom(user, address(this), amount);
