...
import {PercentageMath} from '../../../protocol/libs/math/PercentageMath.sol';
...
contract ParaSwapLiquiditySwapAdapter is BaseParaSwapSellAdapter, ReentrancyGuard {
   ...
   * The user should give this contract allowance to pull the MTokens in order to withdraw the underlying asset and repay the flash loan.
   ...
   * The user should give this contract allowance to pull the MTokens in order to withdraw the underlying asset and perform the swap.
  ...
  function swapAndDeposit(
    ...
    IERC20WithPermit mToken = IERC20WithPermit(
      _getReserveData(address(assetToSwapFrom)).mTokenAddress
    );

    if (swapAllBalanceOffset != 0) {
      uint256 balance = mToken.balanceOf(msg.sender);
      require(balance <= amountToSwap, 'INSUFFICIENT_AMOUNT_TO_SWAP');
      amountToSwap = balance;
    }

    _pullMTokenAndWithdraw(
  ...
  function _swapLiquidity(
    ...
    uint256 balance = mToken.balanceOf(initiator);
    if (swapAllBalanceOffset != 0) {
