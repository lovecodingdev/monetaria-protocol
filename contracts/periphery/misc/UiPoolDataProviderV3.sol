...
import {IMonetariaOracle} from '../../interfaces/IMonetariaOracle.sol';
...
import {MonetariaProtocolDataProvider} from '../../misc/MonetariaProtocolDataProvider.sol';
...
contract UiPoolDataProviderV3 is IUiPoolDataProviderV3 {
  ...
  {
    IMonetariaOracle oracle = IMonetariaOracle(provider.getPriceOracle());
    IPool pool = IPool(provider.getPool());
    MonetariaProtocolDataProvider poolDataProvider = MonetariaProtocolDataProvider(provider.getPoolDataProvider());
    ...
    for (uint256 i = 0; i < reserves.length; i++) {
      ...
      DataTypes.ReserveConfigMap memory reserveConfigMap = baseData.config;
      ...
      reserveData.debtCeiling = reserveConfigMap.getDebtCeiling();
      reserveData.debtCeilingDecimals = poolDataProvider.getDebtCeilingDecimals();
      (reserveData.borrowCap, reserveData.supplyCap) = reserveConfigMap.getCaps();

      reserveData.isSiloedBorrowing = reserveConfigMap.getSiloedBorrowing();
      ...
      reserveData.borrowableInIsolation = reserveConfigMap.getBorrowableInIsolation();
    }
    ...
    returns (UserReserveData[] memory, uint8)
  {
    IPool pool = IPool(provider.getPool());
    address[] memory reserves = pool.getReservesList();
    DataTypes.UserConfigMap memory userConfig = pool.getUserConfig(user);
    ...
    for (uint256 i = 0; i < reserves.length; i++) {
      ...
      userReservesData[i].scaledMTokenBalance = IMToken(baseData.mTokenAddress).scaledBalanceOf(
        user
      );
